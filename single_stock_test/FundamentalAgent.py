...
class FundamentalAgent:
    ...
    def __init__(self, data_dir: str = "."):
        ...
        # Set data range
        self.start_date = datetime.strptime("2024-12-27", "%Y-%m-%d")  # Start from Dec 27
                ...
                try:
                    with open(filepath, 'r') as f:
                        ...
                        analyses.append({
                            ...
                            'reasoning': analysis_data.get('analysis', '').split('\n')[0][:100]  # First line, truncated
                        })
    ...
    def _filter_data_by_date(self, data: List[Dict], current_date: datetime = None, date_key: str = 'date') -> List[Dict]:
        ...
        for entry in data:
            if date_key in entry:
                # Get both statement date and filing date
                statement_date = datetime.strptime(entry[date_key], "%Y-%m-%d")
                filing_date_str = entry.get("fillingDate", entry[date_key])
                filing_date = datetime.strptime(filing_date_str, "%Y-%m-%d")
                
                # Only check if filing date is before or on our analysis date
                # This prevents look-ahead bias while allowing historical data
    ...
    def _load_stock_data(self, symbol: str, current_date: str = None) -> Optional[Dict]:
        ...
        Args:
            ...
            current_date: Optional date to filter data up to
            
        Returns:
            Dict containing stock data or None if not found
        """
        try:
            print(f"DEBUG: Loading stock data for {symbol} up to {current_date}")
            ...
            print(f"DEBUG: Raw financial data found - Income: {len(data.get('income_statements', []))}, Balance: {len(data.get('balance_sheets', []))}, Cash Flow: {len(data.get('cash_flow_statements', []))}")
            ...
            print(f"DEBUG: After filtering - Income: {len(data['income_statements'])}, Balance: {len(data['balance_sheets'])}, Cash Flow: {len(data['cash_flow_statements'])}")
    ...
    def analyze_fundamentals(self, symbol: str, current_date: str = None) -> Optional[Dict]:
        ...
        try:
            ...
            if not (has_income or has_balance or has_cash_flow):
                ...
                return self._analyze_price_only(data)
            ...
            analysis_result = {
                ...
                'data_range': {
                    ...
                    'end': self.cutoff_date.isoformat()
                },
                'metrics': {
...
"""
    ...
    def _analyze_price_only(self, data: Dict) -> Dict:
...
"""
            ...
            # Parse response
            analysis_parsed = self._parse_price_only_response(response)
