...
This framework exists to stress-test multi-agent trading (MAS) with a simple, rigid REST API based architecture. The goal was to evaluate responses and LLM noise when analyzing financial data, 3 sub-agents dedicated to specific data submit their analysis to the deciding Reasoning agent producing the final decision. Since free/cheap news APIs provide poor historic coverage, I manually scraped both general and ticker-specific news for the backtest window so the Sentiment Agent’s influence is actually present in the Reasoning Agent’s analysis.

It’s tailored to this specific 20-stock run and not packaged for easy OSS reuse. The previous design surfaced some noise between the Reasoning Agent and Portfolio Manager; despite some insightful LLM outputs, the end-to-end behavior is suboptimal. I’ve now built a new MCP-based system, with tool-calling and database querying with easy integration and it's substantially more configureable (a lot less harcoded)  - https://github.com/1carlito/stock-wars .
