...
class ParallelBacktest:
    ...
    def __init__(self, data_dir='.', start_date=None, end_date=None, lookback_window=4,
        ...
        self.portfolio_manager = PortfolioManagerAgent(data_dir=data_dir, auto_save=False)
        ...
        self.logger.info(f"- Gemini API keys available: {len(self.api_keys)}")
    ...
    def _load_api_keys(self):
        """Load API keys from environment variables for ReasoningAgent (Gemini API keys)"""
        from dotenv import load_dotenv
        load_dotenv()
        
        keys = []
        
        # Try to load numbered Gemini API keys (GEMINI_API_KEY_*)
        for i in range(1, 7):  # Support up to 6 API keys
            key = os.getenv(f"GEMINI_API_KEY_{i}")
            if key:
                keys.append(key)
        
        # If still no keys, try the default Gemini API key
        if not keys:
        ...
        if not keys:
            raise ValueError("No Gemini API keys found. Set GEMINI_API_KEY_1 through GEMINI_API_KEY_6 in .env")
        
        # Note: Logger not available yet during initialization, will log later
        print(f"Loaded {len(keys)} Gemini API key(s) for ReasoningAgent")
        return keys
    ...
    def _load_previous_decisions(self, symbol, current_date):
        ...
        try:
            decisions_dir = os.path.join(self.data_dir, 'reasoning_decisions_Gemini')
    ...
    def _load_previous_portfolio_decisions(self, current_date):
        ...
        try:
            decisions_dir = os.path.join(self.data_dir, 'portfolio_decisions_Gemini')
            if not os.path.exists(decisions_dir):
                return []
